...
function cards() {
  ...
  class MenuCard {
    constructor(src, altimg, title, description, cost, parentSelector) {
      this.src = src;
      this.altimg = altimg;
      this.title = title;
  ...
  Object(_services_services__WEBPACK_IMPORTED_MODULE_1__["getResources"])('http://localhost:3000/menu').then(data => {
    data.forEach((_ref) => {
      let {
        img,
        altimg,
...
function forms() {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', event => {
      event.preventDefault(); // Вставляем спинер вместо загрузки

      const statusMessage = document.createElement('img');
      statusMessage.src = message.loading;
      statusMessage.style.cssText = "\n                display: block;\n                margin: 0 auto;\n            ";
      form.insertAdjacentElement('afterend', statusMessage);
      const formData = new FormData(form); // Устаревший метод
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); // Оставил для старого метода
      ...
      Object(_services_services__WEBPACK_IMPORTED_MODULE_5__["postData"])('http://localhost:3000/requests', json).then(data => {
        console.log(data);
        showThanksModal(message.success);
        statusMessage.remove();
      }).catch(() => {
  ...
  function showThanksModal(message) {
    ...
    _modal__WEBPACK_IMPORTED_MODULE_4__["modal"].style.display = 'block';
    document.body.style.overflow = 'hidden';
    const thanksModal = document.createElement('div');
    thanksModal.classList.add('modal__dialog');
    thanksModal.innerHTML = "\n            <div class=\"modal__content\">\n                <div class=\"modal__close\" data-close>&times;</div>\n                <div class=\"modal__title\">".concat(message, "</div>\n            </div>\n        ");
    _modal__WEBPACK_IMPORTED_MODULE_4__["modal"].append(thanksModal); // Возвращаем старое окно, чтобы пользователь мог снова отправить данные

    setTimeout(() => {
      thanksModal.remove();
      prevModalDialog.style.display = 'block';
      _modal__WEBPACK_IMPORTED_MODULE_4__["modal"].style.display = 'none';
...
function modalWindow() {
  ...
  modalTrigger.forEach(item => {
    item.addEventListener('click', () => {
      ...
      document.body.style.overflow = 'hidden'; // Если пользователь сам открыл окно, то через время оно уже не появится
  ...
  const modalInterval = setTimeout(() => {
    modal.style.display = 'block';
    document.body.style.overflow = 'hidden';
  }, 5000); // Покажем модалку после скролла только 1 раз
  ...
  window.addEventListener('scroll', showModalByScroll); // Выполнить событие 1 раз, писать после функции в eventListener
  // {once: true}
}
...
function slider() {
  ...
  let slideIndex = 1; // Размер на который сдвигаем слайд
  ...
  sliderField.style.transition = 'all .2s linear'; // Все слайды за контейнером слайдера скрыты
...
function timer() {
  ...
  function getTimeRemaining(endtime) {
    ...
    const t = Date.parse(endtime) - Date.parse(new Date()); // 1000 миллисекунд в минуте, 60 минут в часе, 24 часа в сутках
  ...
  function setClock(selector, endtime) {
    ...
    const timeInterval = setInterval(updateClock, 1000); // Чтобы не ждать секунду и не было мигания
...
const getResources = async url => {
  const result = await fetch(url); // Если с запросом что-то не так, то выкидываем новую ошибку
